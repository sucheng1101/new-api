...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		processChannelError(c, *types.NewChannelError(channel.Id, channel.Type, channel.Name, channel.ChannelInfo.IsMultiKey, common.GetContextKeyString(c, constant.ContextKeyChannelKey), channel.GetAutoBan()), newAPIError, nil)
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, relayInfo *relaycommon.RelayInfo) {
	// relayInfo 供官方 rc.37 的 LogOther 审计使用；Fork 的错误日志仍为 map 路径，暂不消费。
	_ = relayInfo
...
func RelayTask(c *gin.Context) {
	...
	// Skye：运营/性能观测钩子（原 RelayTask defer 语义）
	if taskErr != nil || outcome != nil {
		gopool.Go(func() {
...
func executeTaskSubmissionWith(
	...
	// Preserve the billing ledger request identity for asynchronous settlement
	// and promotion-wallet accounting.
	...
	// Skye：渠道账本记账时间与标准口径预扣（写日志后落库到任务行）
