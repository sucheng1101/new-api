...
func TestUploadTaskPluginRejectsUnexpectedKeyForVersionUpdate(t *testing.T) {
	setupTaskPluginControllerTest(t)
	const uploadedKey = "unexpected-version-key"
	cleanupTaskPluginControllerRuntime(t, uploadedKey)

	source := taskPluginControllerTestSource(uploadedKey, "2.0.0")
	body, err := common.Marshal(map[string]any{
		"source":       source,
		"expected_key": "existing-plugin",
	})
	require.NoError(t, err)
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	...
	assert.Contains(t, recorder.Body.String(), "plugin key does not match expected_key")
	var count int64
	require.NoError(t, model.DB.Model(&model.TaskPlugin{}).Where("key = ?", uploadedKey).Count(&count).Error)
	assert.Zero(t, count)
}

func TestListTaskPluginsIncludesFactoryChannelAndTaskUsage(t *testing.T) {
	setupTaskPluginControllerTest(t)
	baseURL := "https://example.com"
	...
	assert.Equal(t, int64(1), item.InFlightCount)
}
...
func TestListTaskPluginsMarksPreloadedPromptHubsAsThirdParty(t *testing.T) {
	setupTaskPluginControllerTest(t)
...
func TestGetTaskPluginMarksPreloadedPromptHubsAsThirdParty(t *testing.T) {
	setupTaskPluginControllerTest(t)
	recorder := httptest.NewRecorder()
	context, _ := gin.CreateTestContext(recorder)
	context.Params = gin.Params{{Key: "key", Value: "prompt-hubs"}}
	context.Request = httptest.NewRequest(http.MethodGet, "/api/plugin/task/prompt-hubs", nil)

	GetTaskPlugin(context)
...
func TestUpdateTaskPluginMarketplaceSourcesValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			body:    `[{"name":"Official","index_url":"https://one.example.com/index.json"},{"name":" official ","index_url":"https://two.example.com/index.json"}]`,
			wantErr: "marketplace source name must be unique",
		},
		{
			name:    "duplicate source URL",
			body:    `[{"name":"Official","index_url":"https://plugins.example.com/index.json"},{"name":"Mirror","index_url":"https://PLUGINS.example.com/index.json"}]`,
			wantErr: "marketplace source index_url must be unique",
		},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
