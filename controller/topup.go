	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if topUp.PaymentMethod != verifyInfo.Type {
				...
				updates["payment_method"] = verifyInfo.Type
			}
			dAmount := decimal.NewFromInt(int64(topUp.Amount))
			dQuotaPerUnit := decimal.NewFromFloat(common.QuotaPerUnit)
			quotaToAdd := int(dAmount.Mul(dQuotaPerUnit).IntPart())
			settled, newlySettled, err := model.SettleTopUpSuccess(topUp.TradeNo, model.PaymentProviderEpay, quotaToAdd, c.ClientIP(), updates)
			if err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 统一结算失败 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d error=%q topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, err.Error(), common.GetJsonString(topUp)))
				return
			}
			if !newlySettled {
				return
			...
			model.RecordTopupLog(settled.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), settled.Money), c.ClientIP(), settled.PaymentMethod, "epay")
		}
	} else {
