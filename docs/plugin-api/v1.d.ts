...
export interface RequestDescriptor {url: string; method?: string; headers?: Record<string, string>; /** JSON body may contain FilePlaceholder objects at any depth; the host replaces each with a Base64 or data-URL string. */ body?: unknown; credentialless?: boolean; /** Content hooks only: allow a GET/HEAD gateway request with credentials to follow a cross-origin redirect after the host strips plugin headers and preserves only safe client media headers. */ dropCredentialsOnRedirect?: boolean; action?: string; model?: string; rewriteModel?: string; bodyType?: "json" | "multipart"; parts?: readonly {name: string; value?: unknown; fileRef?: string; filename?: string}[]}
