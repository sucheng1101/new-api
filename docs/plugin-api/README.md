...
Some providers require authentication at a stable gateway content endpoint and
then redirect to a public signed CDN URL. Only `buildContentRequest(ctx)` may
explicitly set `dropCredentialsOnRedirect: true` for a GET/HEAD request without
a body. The initial request still follows normal provider-host validation and
can include plugin headers. On a cross-origin redirect, the host clears every
plugin-supplied header, preserves only the caller's safe Range/conditional
headers, and applies SSRF checks again. This flag cannot be combined with
`credentialless` and does not relax redirect handling for any other request.
