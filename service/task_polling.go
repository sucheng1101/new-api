...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	if task == nil || taskResult == nil { // Skye：nil 保护
		return false
	}

	// Token 用量是独立统计维度，不能因按次计费或 adaptor 差额结算提前返回而丢失。（Skye）
	...
	if bc := task.PrivateData.BillingContext; bc != nil && bc.TieredSnapshot != nil {
		...
		if bc.TieredSnapshot.GroupRatio > 0 {
			standardActualQuota = int(math.Round(float64(result.ActualQuotaAfterGroup) / bc.TieredSnapshot.GroupRatio))
		}
		RecalculateTaskQuota(ctx, task, result.ActualQuotaAfterGroup, standardActualQuota, "任务用量表达式结算", result.Clamp)
		return true
	}
