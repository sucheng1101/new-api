...
type TaskPrivateData struct {
	...
	// 渠道结算身份：异步任务只持久化安全指纹与 key index，不保存原始选中 key。（Skye）
	...
	// ChannelStandardQuota 渠道限额标准口径预扣用量（不含分组倍率）。（Skye）
...
func (p TaskPrivateData) Value() (driver.Value, error) {
	if p.Key == "" && p.UpstreamTaskID == "" && p.ResultURL == "" &&
		p.Execution == nil && p.BillingSource == "" && p.BillingRequestId == "" && p.SubscriptionId == 0 &&
...
func InitTask(platform constant.TaskPlatform, relayInfo *commonRelay.RelayInfo) *Task {
	...
	if relayInfo != nil && relayInfo.ChannelMeta != nil {
		...
		if selectedKey, keyIndex, ok := relayInfo.GetChannelUsageIdentity(); ok { // Skye：渠道结算身份
...
// UpdateQuota 更新任务最终额度（Skye：单列落库，保证 CAS 后账实一致）。
