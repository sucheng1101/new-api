...
func PreviewModelPricingConversion(name string, draft PricingValues) (*ModelPricingConversion, error) {
		...
		if channel.ModelMapping != nil && *channel.ModelMapping != "" {
			...
			for _, candidate := range candidates {
				if mapping[candidate] == "" {
					mapping[candidate] = mapping[pricingBaseModelName(candidate)]
				}
			}
			var cycle bool
	...
	for _, entry := range metadata {
		if entry.Endpoints == "" || !slices.ContainsFunc(names, func(name string) bool {
			return modelMetadataMatches(entry, name)
		}) {
			continue
		}
	...
	if fixedPrice {
		...
		if preview.BillingDetails.ImageCount {
			...
			if aliPromptExtend {
				preview.BillingDetails.RequestRules = append(preview.BillingDetails.RequestRules, LegacyPricingRule{`param("parameters.prompt_extend") == true`, 2})
			}
...
func pricingBaseModelName(name string) string {
	if _, base := hostreasoning.ParseOpenAIReasoningEffortFromModelSuffix(name); base != name {
		return base
	}
	return name
}

func modelMetadataMatches(metadata Model, name string) bool {
	switch metadata.NameRule {
	case NameRulePrefix:
		return strings.HasPrefix(name, metadata.ModelName)
	case NameRuleSuffix:
		return strings.HasSuffix(name, metadata.ModelName)
	case NameRuleContains:
		return strings.Contains(name, metadata.ModelName)
	default:
		return name == metadata.ModelName
	}
}
