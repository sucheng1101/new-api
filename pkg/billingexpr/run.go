...
// maxImageN caps the image generation count (值取自官方 relaykit/dto.MaxImageN=128，
// 此处内联以避免引入 relaykit 依赖).
const maxImageN = 128
...
func runProgram(prog *vm.Program, requestRules []RequestRuleTrace, usedVars map[string]bool, params TokenParams, request RequestInput) (float64, TraceResult, error) {
	...
	if usedVars["image_count"] {
		...
		if imageCount < 1 || imageCount > maxImageN {
			return 0, trace, fmt.Errorf("image_count must be between 1 and %d", maxImageN)
		}
